...
public class CategoryController {
    ...
    public Response<Category> createCategory(@RequestBody Category category) {
        ...
        if (category.getId() != null) {
            return Response.createSuc(category);
        } else {
            return Response.createErr(CREATE_CATEGORY_ERROR);
        }
    }

    @PatchMapping("/{id}")
    @ApiOperation(value = "增量更新目录", notes = "增量更新目录")
    public Response<Category> updateCategorySelective(@PathVariable(name = "id") Integer id,
        ...
        if (categoryService.updateCategorySelective(category) == 1) {
            return Response.createSuc(categoryService.getCategoryById(id));
        } else {
    ...
    public Response<Category> updateCategoryAll(@PathVariable(name = "id") Integer id,
                                                @RequestBody Category category) {
        if (categoryService.getCategoryById(id) == null) {
            return Response.createErr(CATEGORY_NOT_EXIST_ERROR);
        }
    ...
    public Response<Category> getCategoryById(@PathVariable(name = "id") Integer id) {
    ...
    public Response<ReturnPage<Category>> getArticleListByCategoryId(@RequestParam(value = "keyword", required = false, defaultValue = "") String keyword,
        ...
        ReturnPage<Category> returnPage = ListPageUtil.returnPage(pageInfo);
